...
Promstat.prototype.addMetric = function (metricconf)
{
	...
	/* XXX Make this a real class. */
	this.ps_metrics_requested.push({
	    'pm_name': metricconf.name,	/* metric's name */
	    'pm_metadata': null,	/* metric's metadata (after fetch) */
	    'pm_last_valsbytgt': {},	/* very last value observed, by tgtid */
	    'pm_prev_valsbytgt': {}	/* previous valid value observed */
	});
};
...
/*
 * The PromstatMetricsData class encapsulates the result of polling all targets
 * for one interval.  Interpreting metric data is a little tricky: gauges and
 * counters are both numbers, for example, but they're interpreted very
 * differently.  Besides that, counters are usually used to show deltas over
 * time.  If we scrape at time T1 and find a counter with value V1, and we
